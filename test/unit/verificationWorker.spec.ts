...
import {
  verifyFromJsonInput,
  verifyFromMetadata,
} from "../../services/workers/verificationWorker";
import { VerifyOutput } from "../../services/workers/workerTypes";
import {
  deployFromAbiAndBytecodeForCreatorTxHash,
  DeploymentInfo,
} from "../helpers/helpers";
import { JobErrorData } from "../../services/store/Tables";
import { ChainInstance, Config, loadConfig } from "../../config/Loader";
...
describe("verificationWorker", function () {
  ...
  before(() => {
    ...
    const sourcifyChainInstanceMap = Object.entries(chainMap).reduce(
      (acc, [chainId, chain]) => {
        acc[chainId] = chain.getSourcifyChainObj();
        return acc;
      },
    ...
    piscinaSandbox.stub(Piscina, "workerData").value({
      chains: sourcifyChainInstanceMap,
