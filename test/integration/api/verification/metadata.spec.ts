...
import { assertJobVerification, assertTransformations } from "../../../helpers/assertions";
...
import { CallProtectionTransformation, LibraryTransformation } from "@ethereum-sourcify/lib-sourcify";

chai.use(chaiHttp);

describe("POST /verify/metadata/:chainId/:address", function () {
  ...
  it("should return a 400 when the address is invalid", async function () {
    ...
    const verifyRes = await chai
      .request(serverFixture.server.app)
      .post(`/verify/metadata/${chainFixture.chainId}/${invalidAddress}`)
      .send({
        sources: {
          [Object.keys(chainFixture.defaultContractMetadataObject.sources)[0]]:
            chainFixture.defaultContractSource.toString(),
  ...
  it("should return a 400 when the chain is not found", async function () {
    ...
    const verifyRes = await chai
      .request(serverFixture.server.app)
      .post(
        `/verify/metadata/${unknownChainId}/${chainFixture.defaultContractAddress}`,
      )
      .send({
        sources: {
          [Object.keys(chainFixture.defaultContractMetadataObject.sources)[0]]:
  ...
  it("should store the correct/recompiled metadata even if wrong metadata input yields a match", async () => {
    ...
    const contractRes = await chai
      .request(serverFixture.server.app)
      .get(
        `/contract/${chainFixture.chainId}/${contractAddress}?fields=metadata`,
      );
  ...
  it("should verify a Solidity < 0.5.0 library contract with non-keccak library placeholders and call protection", async () => {
    ...
    const verifyRes = await chai
      .request(serverFixture.server.app)
      .post(`/verify/metadata/${chainFixture.chainId}/${address}`)
      .send({
